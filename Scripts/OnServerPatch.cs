...
{
	[HarmonyPatch(typeof(OnServer))]
	public class OnServerPatch
	{
		[HarmonyPatch("Create", new Type[] { typeof(CreateStructureInstance)})]
		[HarmonyPostfix]
		public static void CreatePostfix (Structure __result, CreateStructureInstance instance)
		{
			...
			//Debug.Log("ChangeStructuresAddon: created found");
			int editIdx = edits.StructureEditList.FindIndex(x => x.StructurePrefabName == __result.PrefabName);
			//Debug.Log("ChangeStructuresAddon: created found");
			//Debug.Log(editIdx);
			...
			if (editIdx != -1)
			{
				...
				foreach (BuildStateData buildStateData in edit.BuildStateDataList)
				{
					...
					BuildState editedBuildState = __result.BuildStates[stateIdx];
					...
					__result.BuildStates[stateIdx] = editedBuildState;
					//(InventoryManager.PrecisionPlaceCursor).BuildStates[stateIdx] = editedBuildState;
					//Debug.Log("ChangeStructuresAddon: Edited created " + edit.StructurePrefabName);
				}
			}
		}
	}
